...
import { CATEGORIAS, fetchLabels, getCachedLabels, getObsTecnica } from '../lib/obsTecnica'
...
export default function ObservacionesVisitaTecnicaDetail() {
  const { id, tipo } = useParams()
  const navigate = useNavigate()
  const { user } = useAuth()
  const esAdmin = user?.rol === 'admin'
  const config = CATEGORIAS.find(c => c.key === tipo)
  const puedeVer = esAdmin || user?.[config?.permisoVer] === true
  const puedeEditar = esAdmin || user?.[config?.permisoEditar] === true
  ...
  async function cargar() {
    ...
    try {
      ...
      if (!a) { setMsg('✗ Apicultor no encontrado'); setLoading(false); return }
      if (!config) { setMsg('✗ Tipo de observación no válido'); setLoading(false); return }
      setApicultor(a)
      const obs = getObsTecnica(a, tipo)
      if (obs.length === 0) {
  ...
  async function guardar() {
    ...
    try {
      const now = new Date().toISOString()
      let actual = apicultor.observaciones_tecnica
      if (!actual || typeof actual !== 'object' || Array.isArray(actual)) actual = {}
      actual = { ...actual, [tipo]: observaciones }
      await db.apicultores.update(apicultor.id, {
        observaciones_tecnica: actual,
        updated_at: now,
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center gap-2">
        <button
          onClick={() => navigate('/observaciones-visita-tecnica')}
        ...
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold">Observaciones: {labelDe(tipo)}</h2>
        </div>
      </div>
      ...
      <div className="card bg-honey-50 border-honey-100 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {CATEGORIAS.filter(c => c.key !== tipo).map(c => {
            const TC = ICONS[c.key] || FileText
            const puedeVerOtro = esAdmin || user?.[c.permisoVer] === true
            if (!puedeVerOtro) return null
            return (
              <button
                key={c.key}
                onClick={() => navigate(`/observaciones-visita-tecnica/${id}/${c.key}`)}
                className="flex items-center gap-1 px-2 py-1 rounded-lg bg-white border text-xs text-gray-700 hover:bg-gray-50"
              >
                <TC className="w-3.5 h-3.5 text-honey-600" /> {labelDe(c.key)}
              </button>
            )
          })}
        </div>
